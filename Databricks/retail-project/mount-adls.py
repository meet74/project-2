...
def mountadls(scope_name,storage_acc_name,container_name):
    ...
    client_id=dbutils.secrets.get(f"{scope_name}","retail-client")
    tenent_id=dbutils.secrets.get(f"{scope_name}","retail-tenant")
    client_secret=dbutils.secrets.get(f"{scope_name}","retail-secret")
